...
__global__ void rope_kernel(
    ...
    int seq_idx = blockIdx.x;         // Sequence index
    ...
    */
    ...
    __shared__ float sin_s[64] ;

    cos_s[pair_idx] = cos[cos_sin_offset] ;
    sin_s[pair_idx] = sin[cos_sin_offset] ;

    __syncthreads() ;

    // vectorized load reinterpret cast 
    float4* q_val_base = reinterpret_cast<float4*>(q + q_base_offset)   ;

    // its not mem coalesced 
    float4* q_val1_vecld = q_val_base + (pair_idx * num_attention_heads_q / 4);
    float4* q_val2_vecld = q_val_base + ((pair_idx + head_dim/2) * num_attention_heads_q / 4);

    float cos_val = cos_s[pair_idx] ;
    float sin_val = sin_s[pair_idx] ;
    ...
    for(int i = 0 ; i < num_attention_heads_q / 4 ; i++ )
    {
        ...
        // as num_attention_heads_k == 8 so it will be covered with two loops 
        if(i < 2){
